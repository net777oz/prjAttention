...
╠───────────────────────────────────────────────────────────────────────────╣
║ SIDE EFFECTS  rich UI 사용 시 콘솔 실시간 업데이트                         ║
║ DEPENDENCY   pipeline → evaler; (windows, utils, torch)                   ║
╚════════════════════════════════════════════════════════════════════════════╝
...
@torch.no_grad()
def eval_model(model, X: torch.Tensor, L:int, desc="eval",
               task:str="regress", bin_rule:str="nonzero", bin_thr:float=0.0,
               tau_for_cls: float = 0.5, heartbeat_sec:int=5):
    """
    반환:
      회귀: (avg_mse, avg_mae, None, None)
      분류: (None, None, y_true_all, y_prob_all)
    ...
    if USE_RICH:
        ...
        if task == "regress":
            ...
            if prob_buf:
                ...
                print(f"[{desc}] F1@τ={tau_for_cls:.3f}={f1:.6f}")
